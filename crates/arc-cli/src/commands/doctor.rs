...
pub async fn run(data_dir: &Path, _config_path: &Path) -> Result<()> {
    ...
    if has_key {
        println!(
            "  {} Anthropic API key valid (claude-sonnet-4-6)",
            check_mark
        );
    } else {
    ...
    println!(
        "  {} OS Path Boundary Isolation enabled (Native)",
        check_mark
    );
    ...
    if session_dir.exists() {
        let size_kb = std::fs::metadata(&session_dir)
            .map(|m| m.len() / 1024)
            .unwrap_or(0);
    ...
    if PathBuf::from("ARC.md").exists() || PathBuf::from(".arc.md").exists() {
        ...
        println!(
            "  {} ARC.md not found in root (default logic mapped)",
            cross_mark
        );
    }
