...
async fn main() -> Result<()> {
    ...
    let log_dir = arc_core::config::ArcConfig::dir()
        .unwrap_or_else(|_| std::path::PathBuf::from(".arc"))
        .join("logs");
    let _telemetry_guard = telemetry::init_telemetry(log_dir, cli.verbose, cli.quiet)
        .unwrap_or_else(|e| {
    ...
    match cli.command {
        ...
        Some(cli::Command::Stats) => {
            let data_dir = arc_core::config::ArcConfig::dir()
                .unwrap_or_else(|_| std::path::PathBuf::from(".arc"));
        ...
        Some(cli::Command::Auth { action }) => match action {
            ...
            cli::AuthAction::Login => {
                arc_core::auth::oauth_google::authenticate_with_oauth(
                    arc_core::credentials::Provider::Gemini,
                )
                .await?;
            },
        ...
        Some(cli::Command::Session { action }) => match action {
            ...
            cli::SessionAction::Resume { id } => {
                handle_history(&mut memory, None, Some(id)).await?
            },
            cli::SessionAction::Delete { id } => {
                handle_history(&mut memory, Some(id), None).await?
            },
        },
        ...
        Some(cli::Command::Chat) | None => {
            if !arc_core::credentials::auth_status_all()
                .iter()
                .any(|s| s.has_api_key || s.has_oauth_access)
            ...
            if let Some(prompt) = cli.prompt {
                if cli.headless && cli.output_format == "json" {
                    ...
                    let provider =
                        arc_providers::anthropic::AnthropicProvider::new(client, "".to_string());
                    ...
                    let response_text = provider
                        .generate_text("claude-3-5-sonnet-20241022", &msgs)
                        .await
...
async fn handle_history(
    memory: &mut arc_core::memory::MemoryManager,
...
fn print_auth_status(_config: &ArcConfig) -> anyhow::Result<()> {
    ...
    for status in statuses {
        ...
        if status.has_oauth_access {
            parts.push("OAuth Access");
        }
        if status.has_oauth_refresh {
            parts.push("OAuth Refresh");
        }
...
async fn set_single_key(provider_str: &str) -> anyhow::Result<()> {
    let provider = arc_core::credentials::Provider::from_str(provider_str).ok_or_else(|| {
        anyhow::anyhow!(
            "Unknown provider: {provider_str}. Use anthropic, openai, gemini, or ollama."
        )
    })?;
