...
impl WindowsSandbox {
    ...
    pub fn apply(&mut self, paths: &[PathBuf]) -> Result<()> {
        ...
        tracing::warn!("Windows Kernel Sandboxing is unavailable on this OS wrapper. Utilizing software-level strict directory boundaries.");
        Ok(())
    }
    ...
    pub fn check_access<P: AsRef<Path>>(&self, target: P) -> Result<()> {
        let target_path = target.as_ref().canonicalize().unwrap_or_else(|_| target.as_ref().to_path_buf());
        ...
        Err(anyhow!("Sandbox Violation: Agent attempted to mutate path {:?} outside of allowed boundaries", target.as_ref()))
    }
}
