...
impl SecretSanitizer {
    ...
    pub fn redact(input: &str) -> String {
        ...
        // Simple JWT pattern
        let jwt_re = JWT_RE.get_or_init(|| Regex::new(r#"ey[A-Za-z0-9-_=]+\.[A-Za-z0-9-_=]+\.?[A-Za-z0-9-_.+/=]*"#).unwrap());
        let pk_re = PRIVATE_KEY_RE.get_or_init(|| Regex::new(r#"-----BEGIN (RSA|EC|DSA|OPENSSH) PRIVATE KEY-----[\s\S]*?-----END \1 PRIVATE KEY-----"#).unwrap());

        let mut output = input.to_string();
        output = aws_re.replace_all(&output, "[REDACTED AWS KEY]").to_string();
        output = jwt_re.replace_all(&output, "[REDACTED JWT]").to_string();
        output = pk_re.replace_all(&output, "[REDACTED PRIVATE KEY]").to_string();
