...
use oauth2::{
    ClientId, RefreshToken, TokenResponse, TokenUrl,
    basic::BasicClient,
...
pub async fn rotate_refresh_token(provider: Provider) -> ArcResult<()> {
    let refresh_token =
        credentials::get_credential(provider, CredentialKind::OAuthRefreshToken)?;
...
pub fn validate_redirect_uri(expected_host: &str, actual_uri: &str) -> ArcResult<()> {
    let parsed =
        url::Url::parse(actual_uri).map_err(|e| crate::error::ArcError::Auth(format!("OAuth failed: {}", e.to_string())))?;
    ...
    if host != expected_host && host != "localhost" && host != "127.0.0.1" && host != "[::1]" {
        return Err(crate::error::ArcError::Auth(format!("Redirect URI mismatch. Expected {}, got {}", expected_host.to_string(), host.to_string()))
        .into());
    }
    ...
    if parsed.scheme() != "http" {
        return Err(
            crate::error::ArcError::Auth(format!("OAuth failed: {}", "Loopback redirect must use http, not https")).into(),
        );
    }
