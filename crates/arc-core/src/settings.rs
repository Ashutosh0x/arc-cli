...
impl LayeredSettings {
    ...
    pub fn load_managed_platform(&mut self) -> Result<(), String> {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if path.exists() {
                self.load_layer(SettingsLayer::Managed, path)?;
            }
        }
        Ok(())
    }
    ...
    fn load_windows_registry(&mut self) -> Result<(), String> {
        ...
        if managed_path.exists() {
            self.load_layer(SettingsLayer::Managed, &managed_path)?;
        }
        Ok(())
    }
    ...
    fn load_macos_plist(&mut self) -> Result<(), String> {
        ...
        let managed_path =
            PathBuf::from("/Library/Application Support/ArcCli/managed-settings.json");
        if managed_path.exists() {
            self.load_layer(SettingsLayer::Managed, &managed_path)?;
        }
        Ok(())
