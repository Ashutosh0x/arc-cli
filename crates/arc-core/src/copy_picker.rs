...
pub fn extract_code_blocks(response: &str) -> Vec<CodeBlock> {
    ...
    for line in response.lines() {
        if line.starts_with("```") && !in_block {
            ...
            blocks.push(CodeBlock { index: idx, language: current_lang.clone(), content, line_count });
            idx += 1;
        } else if in_block {
...
pub enum CopyAction { CopyBlock(usize), CopyAll, WriteToFile(usize) }
...
pub fn format_picker(blocks: &[CodeBlock]) -> String {
    ...
    for block in blocks {
        out.push_str(&format!("  [{}] {} ({} lines)\n", block.index, if block.language.is_empty() { "plain" } else { &block.language }, block.line_count));
    }
...
pub fn parse_copy_input(input: &str, max_idx: usize) -> Option<CopyAction> {
    ...
    if input == "a" { return Some(CopyAction::CopyAll); }
    if let Some(n) = input.strip_prefix('w') {
        if let Ok(idx) = n.parse::<usize>() { if idx <= max_idx { return Some(CopyAction::WriteToFile(idx)); } }
    }
    if let Ok(idx) = input.parse::<usize>() { if idx <= max_idx { return Some(CopyAction::CopyBlock(idx)); } }
    None
}
