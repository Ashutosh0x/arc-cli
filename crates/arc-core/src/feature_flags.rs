...
impl FeatureFlagStore {
    pub fn new(cache_dir: PathBuf) -> Self {
        let mut store = Self { flags: HashMap::new(), cache_path: cache_dir.join("feature_flags.json") };
    ...
    pub fn set(&mut self, name: &str, enabled: bool, description: &str) {
        ...
        let flag = self.flags.entry(name.to_string()).or_insert_with(|| FeatureFlag {
            name: name.to_string(), enabled, default: enabled, description: description.to_string(), last_updated: now
        });
    ...
    pub fn list(&self) -> Vec<&FeatureFlag> { self.flags.values().collect() }

    fn load_cache(&mut self) -> Result<(), String> {
        if !self.cache_path.exists() { return Ok(()); }
    ...
    fn save_cache(&self) -> Result<(), String> {
        if let Some(parent) = self.cache_path.parent() { std::fs::create_dir_all(parent).map_err(|e| e.to_string())?; }
