...
impl AgentDefinitionRegistry {
    ...
    fn parse_agent(path: &Path) -> Result<AgentDefinition, String> {
        ...
        if parts.len() < 3 { return Err("Missing frontmatter".into()); }
        let mut def: AgentDefinition = serde_yaml::from_str(parts[1].trim()).map_err(|e| e.to_string())?;
        def.instructions = parts[2].trim().to_string();
        def.file_path = path.to_path_buf();
        if def.name.is_empty() {
            def.name = path.file_stem().and_then(|s| s.to_str()).unwrap_or("unknown").to_string();
        }
        Ok(def)
    }

    pub fn get(&self, name: &str) -> Option<&AgentDefinition> { self.agents.get(name) }
    pub fn list(&self) -> Vec<&AgentDefinition> { self.agents.values().collect() }
}

impl Default for AgentDefinitionRegistry { fn default() -> Self { Self::new() } }
