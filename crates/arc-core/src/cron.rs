...
impl CronJob {
    pub fn new(id: &str, interval: Duration, prompt: &str) -> Self {
        Self { id: id.to_string(), interval, prompt: prompt.to_string(), max_iterations: None, iterations_run: 0, enabled: true, last_run: None }
    }

    pub fn should_run(&self) -> bool {
        if !self.enabled { return false; }
        if let Some(max) = self.max_iterations { if self.iterations_run >= max { return false; } }
        match self.last_run { Some(last) => last.elapsed() >= self.interval, None => true }
    }

    pub fn record_run(&mut self) { self.iterations_run += 1; self.last_run = Some(Instant::now()); }
}
...
impl CronScheduler {
    pub fn new() -> Self { Self { jobs: Vec::new(), enabled: true } }

    /// Parse interval string like "5m", "30s", "1h".
    pub fn parse_interval(s: &str) -> Result<Duration, String> {
        let s = s.trim();
        let (num_str, unit) = s.split_at(s.len().saturating_sub(1));
        let num: u64 = num_str.parse().map_err(|_| format!("Invalid interval: {s}"))?;
        match unit {
    ...
    pub fn mark_run(&mut self, id: &str) {
        if let Some(job) = self.jobs.iter_mut().find(|j| j.id == id) { job.record_run(); }
    }

    /// Cancel a job.
    pub fn cancel(&mut self, id: &str) { self.jobs.retain(|j| j.id != id); }

    /// Cancel all jobs.
    pub fn cancel_all(&mut self) { self.jobs.clear(); }

    /// Disable all scheduling.
    pub fn disable(&mut self) { self.enabled = false; }

    pub fn jobs(&self) -> &[CronJob] { &self.jobs }
}
