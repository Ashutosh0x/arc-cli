...
pub enum TaskStatus { Running, Completed, Failed, Killed }
...
impl BackgroundTaskManager {
    ...
    pub fn spawn(&mut self, command: &str) -> String {
        ...
        let task = BackgroundTask {
            id: id.clone(), command: command.to_string(), status: TaskStatus::Running,
            output_bytes: 0, pid: None,
    ...
    pub fn kill(&mut self, id: &str) {
        if let Some(task) = self.tasks.get_mut(id) { task.status = TaskStatus::Killed; }
    }

    pub fn kill_all(&mut self) {
        for task in self.tasks.values_mut() { if task.status == TaskStatus::Running { task.status = TaskStatus::Killed; } }
    }

    pub fn running(&self) -> Vec<&BackgroundTask> { self.tasks.values().filter(|t| t.status == TaskStatus::Running).collect() }
    pub fn get(&self, id: &str) -> Option<&BackgroundTask> { self.tasks.get(id) }
    pub fn all(&self) -> Vec<&BackgroundTask> { self.tasks.values().collect() }
}

impl Default for BackgroundTaskManager { fn default() -> Self { Self::new() } }
