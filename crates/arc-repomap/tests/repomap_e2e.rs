...
fn evaluate_repomap_token_reduction() {
    ...
    println!("Naive Full File Context Size: {} characters", initial_character_count);
    println!("Repomap Compressed Context Size: {} characters", compressed_character_count);
    
    let reduction_percentage = 100.0 - ((compressed_character_count as f64 / initial_character_count as f64) * 100.0);
    println!("Reduction Ratio: {:.2}%", reduction_percentage);
    
    assert!(compressed_character_count < initial_character_count, "Repomap must strictly reduce token limits over raw source arrays");
    
    // Validate AST extracted specific structural definitions accurately.
    assert!(map_output.contains("pub struct RepoMap"), "AST structural definition missing: 'pub struct RepoMap'");
    assert!(map_output.contains("pub fn generate_map"), "AST method signature missing: 'pub fn generate_map'");
}
